...
//글자 종류들 타입
export type LetterName = "hangul" | "alphanumeric" | "kana" | "cyrillic";

export const getWritingSystem = (writingSystem: LetterName) => {
...
//키릴
export const cyrillic: WritingSystem = {
    letterName: "cyrillic",
    letterNameKorean: "키릴 문자",
    defaultName: "Снежа",
    filterRegex: /[^ЁА-яё]/g,
    codeRange: [[0x0401, 0x0401], [0x0410, 0x044F], [0x0451, 0x0451]],
    letterCount: 33 + 33,
    letterCountText: "33자(대문자) + 33자(소문자)",
    warnLength: 5,

    generateRandomCharacter: (): string => {
        const minUnicode = Math.ceil(0x0410 - 1);
        const maxUnicode = Math.floor(0x044F + 1);
